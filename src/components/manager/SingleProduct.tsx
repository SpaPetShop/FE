...
export default function SingleProduct({ data }: SingleProductProps) {
  ...
  return (
    <Grid item xs={4} sm={3} md={3} lg={2}>
      <Card
        sx={{ maxWidth: 400 }}
        onClick={() => navigate(`/manager-manage-Product/${data.id}`)}
      >
        <CardActionArea>
        <Carousel sx={{ margin: 'auto', border: 'none' }}
          ...
          {data.image.length > 0 ?
            data.image.map((i, index) => (
              <CardMedia key={index} component='img' height={200} image={i.imageURL} alt='Product img' sx={{ objectFit: 'cover' }} />
            ))
            :
            <CardMedia component='img' height={200} image={"/logo.png"} alt='Product img' sx={{ objectFit: 'cover' }} />
          }
        </Carousel>
          <CardContent>
